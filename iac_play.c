/*
IAC Player v1.3 - Imprecision Audio Codec Player (Multithreaded Channel-Parallel Edition)
...
// (ChunkCheckpoint removido: a seek_table do próprio arquivo .iac substitui o
// checkpoint construído em runtime — seek funciona em qualquer ponto, mesmo
// antes de a reprodução ter passado por ali.)
...
// Lê o cabeçalho de seek_table do canal (gravado pelo encoder antes do bitstream)
// e posiciona o BitStream logo após ele, pronto para leitura sequencial.
...
// Rebobina o BitStream para o início real dos dados (pulando a seek_table, que já
// está carregada na memória) sem precisar re-ler/realocar a tabela do disco.
// Usado no loop de repetição do arquivo (file_sample_idx == 0).
void rewind_bitstream(BitStream *bs) {
    fseek(bs->file, bs->bitstream_start_pos, SEEK_SET);
    bs->bytes_read_so_far = 0;
    bs->byte_count = 0;
    bs->byte_pos = 0;
    bs->current_byte = 0;
...
// Reposiciona o BitStream em um byte absoluto do arquivo (entrada da seek_table)
// e zera o estado de bits — usado para os saltos rápidos de seek.
...
// ============================================================================
// ESTRUTURA DO PLAYER (ATUALIZADA PARA MULTITHREAD POR CANAL)
// ============================================================================
...
void *channel_worker_func(void *arg) {
    ...
    while (ip->running) {
        // Aguarda ordens do Núcleo Organizador
        ...
        // Executa o processamento do chunk inteiro de 5 segundos de forma isolada
        ...
        for (uint64_t f = 0; f < ip->chunk_frames; f++) {
            ...
            // Tratamento invisível do Looping no arquivo
            if (file_sample_idx == 0 && current_virtual_frame > 0) {
                rewind_bitstream(&ip->channel_streams[c]);
            ...
            // (Checkpoint de seek removido: a seek_table do arquivo já cobre isso,
            // com offsets byte-exatos calculados pelo encoder.)

            // Descompressão da amostra via Rice
...
// ============================================================================
// MASTER THREAD (NÚCLEO ORGANIZADOR): DETECTA ESPAÇO E SINCRONIZA A BARREIRA
// ============================================================================
void *decode_thread_func(void *arg) {
    ...
    while (ip->running) {
        ...
        // Garante que o Seek não vai interromper este ciclo pela metade
        ...
        // Define o descanso baseado no estado do buffer
        if (filled_frames + ip->chunk_frames > future_target) {
            THREAD_SLEEP_MS(10);
        } else {
            THREAD_SLEEP_MS(1);
...
void update_timer(iacplay_desc *ip) {
    ...
    printf("\r%.2f / %.2f sec (Buffer: %04.1fs)", current, duration, (double)(ip->write_cursor - ip->playback_cursor) / ip->header.sample_rate);
    fflush(stdout);
}
...
iacplay_desc *iacplay_open(const char *path) {
    ...
    // Inicializa os arquivos paralelos, buffers, checkpoints e DISPARA AS THREADS WORKERS
    for (int i = 0; i < ip->header.channels; i++) {
        ...
        // Configuração da infraestrutura POSIX de sincronismo do canal
        ...
        // Empacota os argumentos da thread trabalhadora
        ip->worker_args[i].ip = ip;
        ip->worker_args[i].channel_idx = i;

        // Dispara a thread trabalhadora do canal i
    ...
    // Dispara a thread Organizadora principal (Master)
...
void iacplay_close(iacplay_desc *ip) {
    ...
    // Acorda todas as threads de canal adormecidas para que elas possam sair do loop de execução
    for (int i = 0; i < ip->header.channels; i++) {
    ...
    // Aguarda encerramento definitivo da Thread Mestra (Organizador)
    pthread_join(ip->decode_thread, NULL);

    pthread_mutex_destroy(&ip->master_mutex);

    // Aguarda e limpa os recursos de cada canal individualmente
    for (int i = 0; i < ip->header.channels; i++) {
        ...
        if (ip->channel_streams[i].seek_table) free(ip->channel_streams[i].seek_table);
    }
...
int main(int argc, char **argv) {
    ...
    saudio_setup(&(saudio_desc){
        .sample_rate = ip->header.sample_rate,
        .num_channels = ip->header.channels,
